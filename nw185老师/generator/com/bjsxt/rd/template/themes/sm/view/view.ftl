	...
	<script type="text/javascript" src="${r"${pageContext.request.contextPath}"}/resources/easyui/script/jquery.min.js"></script>
  	...
  	<script type="text/javascript">
  		...
  		function initForm(){
  			...
  			<#list view.form as p>
  			<#if p.type=="combobox">
  			//初始化${p.title}下拉列表框,放开注释修改url/textField/valueField即可
  			$("#${p.property}").combobox({
  				url : "" , //这里写${p.title}请求地址 
  				textField : "" , 
  				valueField : "" ,
  				value : -1
  			});
		...
		//这里是分页查询的代码
  		...
  		$(function(){
  			var dialogHeight = ${view.form?size} * 50 + 100;
  			//新增按钮的处理
   			...
   			<table style="width:100%">
   				...
   				<tr>
   					...
   					<td>
   						...
   						<#if p.type=="combobox">
   						<select id="${p.property}" name="${p.property}" required="${p.required?string('true','false')}" validType="val_${p.property}" class="easyui-combobox" editable="false"></select>
   						</#if>
   						<#if p.type=="date">
   						...
   						<#if p.type="file">
   						<input id="${p.property}" name="${p.property}" required="${p.required?string('true','false')}" validType="val_${p.property}" class="easyui-filebox"/>
   						</#if>
   					</td>
   				</tr>
   				</#list>
