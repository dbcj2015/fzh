...
<html>
	...
	<script type="text/javascript" src="${pageContext.request.contextPath}/resources/easyui/script/jquery.min.js"></script>
  	...
  	<script type="text/javascript">
		...
		//这里是分页查询的代码
  		$(function(){
  			$("#grid").datagrid({
  				url : "${pageContext.request.contextPath}/asset/groupStoreAsset?branchId=${loginuser.branch_id}",
  				columns : [[
  		...
  		$(function(){
  			...
  			$("#btnAdd").click(function(){
  				...
  				if(sel == null){
  					$.messager.alert("警告" , "请选择要领用的设备" , "warning");
  					return;
  				}
	  		...
	  		$(function(){
	  			$("#btnSave").click(function(){
	  				$("#frm").form("submit" , {//form是easyui提供的扩展函数,用于对表单进行处理
	  					url : "${pageContext.request.contextPath}/asset/takeout" ,//form会自动通过ajax的方式向服务器提交数据
  ...
  <body>
   	<table id="grid"></table>
   	<div id="tb">
   		<a id="btnAdd" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add'">领用登记</a>
   	</div>
   		...
   		<form id="frm" method="post">
   			<table style="width:100%">
				<tr>
   				...
   				<tr>
   					<td style="height:30px;width:70px">领用数量</td>
   					<td>
   						<span id="txtStoreCnt">1台</span>
   					</td>
   				</tr>
   				<tr>
   					<td style="width:70px">SN编码</td>
   					<td>
   						<input name="sn" class="easyui-textbox" required="true">
   					</td>
   				</tr>
   				<tr>
   					<td style="width:70px">使用人</td>
   					<td>
   						<select id="cmbUserId" name="userId" class="easyui-combobox" required="true">
   				...
   				<tr>
   					<td style="width:70px">领用日期</td>
   					<td>
   						<input name="takeoutTime" class="easyui-datebox" editable=false required=true>
   					</td>
   				</tr>
   				<tr>
   					<td style="width:70px">预计归还时间</td>
   					<td>
   						<input name="takebackTime" class="easyui-datebox" >
   					</td>
   				</tr>
   			</table>
   		</form>
