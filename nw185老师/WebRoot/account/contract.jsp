		        	...
		        	if(value.length != 14){
		        		...
		        		$.ajax({
		        			...
		        			success : function(json){
		        				console.info("AJAX服务器返回了数据" , json);
		        				if(json.length != 0){//设备编号已存在
		        					result = false;  //注意返回值的作用于,设置到validator这个函数层面上
		        				}else{
		        					result = true;
		        				}
		        			}
		        		});
		        		console.info("这句话期望在AJAX函数后执行");
		            	return result;
		        	}
		        },   
		        message: "合同编号违法或重复"  
   		...
   		<form id="frm" action="${pageContext.request.contextPath}/contract/create" method="post" enctype="multipart/form-data">
   			<table style="width:100%">
   				...
   				<tr>
   					...
   					<td>
   						<input id="contractCode" name="contractCode" required="true" class="easyui-textbox" validType="val_contractCode"/>
   					</td>
   				</tr>
   				<tr>
