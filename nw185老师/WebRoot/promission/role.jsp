...
<html>
		...
		//这里是分页查询的代码
  		$(function(){
  			$("#grid").datagrid({
  				...
  				columns : [[
  					{title : "角色编号" , field: "roleId" ,width:"10%"},
	  		...
	  		$(function(){
	  			$("#btnSave").click(function(){
	  				...
	  				$("#frm").form("submit" , {//form是easyui提供的扩展函数,用于对表单进行处理
	  					url : "${pageContext.request.contextPath}/role/" + opmode ,//form会自动通过ajax的方式向服务器提交数据
	  					onSubmit: function(){    
  		...
  		$(function(){
  			$("#btnDelete").click(function(){
  				...
  				$.messager.confirm("询问" , "确定要删除选中的记录吗?" , function(r){
  					if(r== true){
  						$.ajax({
  							url : "${pageContext.request.contextPath}/role/delete?id=" + selectRec.roleId,
  							type : "get",
  							dataType : "json" , 
  		...
  		$(function(){
  			...
  			//授权按钮
  			$("#btnFunctionsAuth").click(function(){
  				...
  				$("#dlgRoleFunctions").dialog({ //弹出一个宽200高300的新增员工对话框
  					title : "功能授权" , 
  					width : 400,
  					height : 400,
  					modal : true,
	  				buttons : "#dlgFunctionBtns"
  				});
  				//初始化RoleFunction表格
  				$("#grdRoleFunctions").datagrid({
	  				url : "${pageContext.request.contextPath}/role/findRoleFunction?roleId=" + rec.roleId,
	  				columns : [[
	  					{title : "id" , field: "a" ,width:"10%" , checkbox:true},
	  					{title : "模块" , field: "moduleName" ,width:"20%"},
	  					{title : "功能名称" , field: "functionName" ,width:"50%"}
	  					
	  				]],
	  				onLoadSuccess: function(data){
	  					var rowData = data.rows;  
	  	                $.each(rowData,function(idx,row){
	  	                	if(row.has_promission == 1){
	  	                		console.info(idx);
								$("#grdRoleFunctions").datagrid("checkRow" , idx);  						
		  					}
	  	                })
	  				},
	  				border : false,
  			...
  			//保存按钮
  			$("#btnFunctionSave").click(function(){
  				var selFunctions =  $("#grdRoleFunctions").datagrid("getSelections");
  				var functionIds = new Array();//选中FunctionId的数组
  				for(var i = 0 ; i < selFunctions.length ; i++){
  					 functionIds.push(selFunctions[i].functionId);
  				}
  				var roleId = $("#grid").datagrid("getSelected").roleId;
  				//发送请求处理
  				$.ajax({
  					url : "${pageContext.request.contextPath}/role/authFunctions" , 
  					data : {"functionIds" : functionIds.join(",") , "roleId" : roleId},
  					type : "get" , 
  					dataType : "json" ,
  					success : function(json){
  						if(json.result){
		    	        	$.messager.alert('提示',json.message,'info' , function(){
		    	        		$("#dlgRoleFunctions").dialog("close");
		    	        	});
		    	        	$("#grid").datagrid("reload");
		    	        } else {
		    	        	$.messager.alert('错误',json.message,'warning');
  			...
  			//还原功能
  			$("#btnFunctionCancel").click(function(){
  				$("#grdRoleFunctions").datagrid("reload");
  			});
  		});
  	</script>
  </head>
  ...
  <body>
   	...
   	<div id="tb">
   		...
   		<a id="btnFunctionsAuth" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-more'">功能授权</a>
   		...
   		<form id="frm" method="post">
   			<table style="width:100%">
   				...
   				<tr>
   					...
   					<td>
   						<input id="roleName" name="roleName" required="true" class="easyui-textbox" validType="val_roleName"/>
   					</td>
   				</tr>
   			</table>
   		</form>
   	...
   	<div id="dlgFunctionBtns">
   		<a id="btnFunctionSave" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-ok'">保存</a>
   		<a id="btnFunctionCancel" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-cancel'">还原</a>
   	</div>
   	
   	<div id="dlgRoleFunctions">
   		<table id="grdRoleFunctions"></table>
   	</div>
  </body>
</html>
